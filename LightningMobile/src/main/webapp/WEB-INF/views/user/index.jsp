...
-->
				...
				<div id="newProductCar" class="carousel slide">
					<div class="carousel-inner">
						<c:if test="${ products.size() > 0 }">
							<div class="item active">
								<ul class="thumbnails">
									<c:forEach var="item" items="${ products }" varStatus="itemS">
										<li class="span3">
											<div class="thumbnail">
												<a class="zoomTool" href="<c:url value="/product-details/${ item.id_product }" />"
													title="add to cart"><span class="icon-search"></span>
													XEM NHANH</a> <a href="#" class="tag"></a> <a
													href="<c:url value="/product-details/${ item.id_product }" />"><img
													src="assets/user/img/${ item.img }" alt="bootstrap-ring"></a>
											</div>
										</li>
										<c:if
											test="${ (itemS.index + 1) % 4 == 0 || (itemS.index + 1) == products.size() }">
								</ul>
							</div>
							<c:if test="${ (itemS.index + 1) < products.size() }">
								<div class="item">
									<ul class="thumbnails">
							</c:if>
						</c:if>
	...
	-->
						...
						<c:forEach var="item" items="${ products }" varStatus="itemS">
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool" href="<c:url value="/product-details/${ item.id_product }" />"
										title="add to cart"><span class="icon-search"></span>XEM
										NHANH</a> <a href="<c:url value="/product-details/${ item.id_product }" />"><img
										src="assets/user/img/${ item.img }" alt=""></a>
									<div class="caption">
										<h5>${ item.name }</h5>
										<h4>
											...
											<a class="shopBtn"
												href="<c:url value="/addtoCart/${ item.id_product }" />"
							...
							<c:if
								test="${ (itemS.index + 1 ) % 3 == 0 || (itemS.index + 1) == products.size() }">
					</ul>
					<c:if test="${ itemS.index < products.size() }">
						<ul class="thumbnails">
					</c:if>
				</c:if>
				</c:forEach>
		...
		<div class="well well-small"
			...
			<a class="btn btn-mini pull-right" href="<c:url value="/all-product/page1" />"
				style="font-size: 1.5rem; padding: 10px 20px;">Tất cả sản phẩm</a>
		</div>
		<!-- <hr>
			<div class="well well-small">
