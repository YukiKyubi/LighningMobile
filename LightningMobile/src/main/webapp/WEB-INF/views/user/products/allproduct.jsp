...
<body>
	...
	<div class="pagination">
		<a href="<c:url value="/all-product/page1" />">&laquo;</a>
		<c:forEach var="item" begin="1" end="${ paginationInfo.totalPages }" varStatus="itemS">
			<c:if test="${ itemS.index == paginationInfo.currentPage }">
				<a href="<c:url value="/all-product/page${ itemS.index }" />" class="active">${ itemS.index }</a>
			</c:if>
			<c:if test="${ itemS.index != paginationInfo.currentPage }">
				<a href="<c:url value="/all-product/page${ itemS.index }" />">${ itemS.index }</a>
			</c:if>
			<c:set var="lastpage" value="${ itemS.index }" />
		</c:forEach>
		<a href="<c:url value="/all-product/page${ lastpage }" />">&raquo;</a>
	</div>
</body>