...
<body>
				...
				<li class="span4">
					<div class="thumbnail">
						<a href="product_details.html" class="overlay"></a> <a
							class="zoomTool" href="<c:url value="/product-details/${ item.id_product }" />" title="add to cart"><span
	...
	<div class="pagination">
		<a href="<c:url value="/products/${ catId }/1" />">&laquo;</a>
		<c:forEach var="item" begin="1" end="${ paginationInfo.totalPages }" varStatus="itemS">
			<c:if test="${ itemS.index == paginationInfo.currentPage }">
				<a href="<c:url value="/products/${ catId }/${ itemS.index }" />" class="active">${ itemS.index }</a>
			</c:if>
			<c:if test="${ itemS.index != paginationInfo.currentPage }">
				<a href="<c:url value="/products/${ catId }/${ itemS.index }" />">${ itemS.index }</a>
			</c:if>
			<c:set var="lastpage" value="${ itemS.index }" />
		</c:forEach>
		<a href="<c:url value="/products/${ catId }/${ lastpage }" />">&raquo;</a>
	</div>
</body>